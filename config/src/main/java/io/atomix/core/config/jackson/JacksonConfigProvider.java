...
public class JacksonConfigProvider implements ConfigProvider {
  ...
  private static class InterpolatingYamlParser extends YAMLParser {
    private static final String SYS_PATTERN = "\\$\\{sys:([A-Za-z0-9-_.]+)\\}";
    private static final String ENV_PATTERN = "\\$\\{env:([A-Za-z0-9_]+)\\}";
    ...
    private String interpolateString(String value) {
      value = interpolate(value, SYS_PATTERN, name -> System.getProperty(name, ""));
      value = interpolate(value, ENV_PATTERN, name -> System.getenv(name));
      return value;
    }

    private String interpolate(String value, String pattern, Function<String, String> supplier) {
